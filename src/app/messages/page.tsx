...
import { MessageSquare, Send, ShieldAlert, CheckCheck, UserX, AlertTriangle } from 'lucide-react';
...
export default function MessagesPage() {
  ...
  const handleSendMessage = () => {
    if (!inputMessage || !selectedUser || blocked) return;
      ...
      <div className="pb-2 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 font-medium">
            Private 1-on-1 communication with your accountability partners and peers.
          </p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-[600px] border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden bg-white dark:bg-slate-900">
        {/* Left: Conversation List */}
        <div className="border-r border-slate-200 dark:border-slate-800 p-3 space-y-2 overflow-y-auto">
          <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 px-2 block">Conversations</span>
            ...
            return (
              <button
                ...
                className={`w-full flex items-center space-x-3 p-3 rounded-xl text-left transition-colors cursor-pointer ${
                  isSelected ? 'bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-500/40' : 'hover:bg-slate-100 dark:hover:bg-slate-800/60'
                }`}
              >
            ...
            <div className="flex items-center space-x-2">
              <button
                ...
                className={`px-2.5 py-1 rounded-lg text-xs font-semibold border transition-colors ${
                  blocked ? 'bg-red-600 text-white border-red-500' : 'border-slate-300 dark:border-slate-700 text-slate-600 dark:text-slate-300'
                }`}
              >
